...
public class VNPayController {
    ...
    public VNPayController(OrderRepository orderRepository, UserRepository userRepository, ProductRepository productRepository) {
    ...
    public Map<String, Object> createPaymentUrl(
        ...
        order.setUser(user);
        // order.setCreatedAt(LocalDateTime.now()); // Handled by @CreationTimestamp in Order entity
    ...
    public RedirectView vnpayReturn(HttpServletRequest request) {
        ...
        if (vnp_SecureHash != null && vnp_SecureHash.equals(generatedHash)) {
            ...
            if (order == null) {
                // If order not found, redirect to an error page or show a message
                return new RedirectView("/payment-error?message=Order not found");
            }

            if ("00".equals(responseCode)) {
                order.setStatus(PaymentStatus.SUCCESS);
                orderRepository.save(order);
                return new RedirectView("/payment-success?orderId=" + order.getOrder_ID());
            } else {
                order.setStatus(PaymentStatus.FAILED);
                orderRepository.save(order);
                return new RedirectView("/payment-error?message=Payment failed with code: " + responseCode);
            }
        } else {
